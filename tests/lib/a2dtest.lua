...
local function EraseClock()
  for row = 0,11 do
    for col = 65,79 do
      apple2.SetDHRByte(row, col, 0x00)
    end
  end
  emu.wait_next_frame()
end
...
function a2dtest.SnapshotDHRWithoutClock()
  EraseClock()
  return apple2.SnapshotDHR()
end

function a2dtest.ExpectUnchangedExceptClock(dhr, message)
  EraseClock()
  test.Expect(apple2.CompareDHR(dhr, true), message, {snap=true}, 1)
end
...
function a2dtest.ExpectNothingChanged(func)
  ...
  func()
  EraseClock()
  test.Expect(apple2.CompareDHR(dhr, true), "nothing should have changed", {snap=true}, 1)
end
...
local function RepaintFraction()
  ...
  return count / (apple2.SCREEN_HEIGHT*80)
end
