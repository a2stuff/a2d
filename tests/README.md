...
* Generic:
  ...
  * `lib/util.lua` - generic utility functions; exposed as a `util` global.
  * `lib/apple2.lua` - utilities for driving Apple II systems in MAME; exposed as an `apple2` global.

* Specific to Apple II DeskTop:
* `lib/a2d.lua` - utilities for driving the GUI; exposed as an `a2d` global.
* `lib/desktop.lua` - utilities for driving the DeskTop module; exposed as an `desktop` global.
* `lib/mgtk.lua` - logic for interrogating MouseGraphics ToolKit state; exposed as an `mgtk` global.
* `lib/a2dtest.lua` - testing helpers; exposed as an `a2dtest` global.
* `lib/ocr_table.lua` - generated by `../bin/build_ocr_table.pl`; used to read text on DHGR screen.
...
* The API in `a2d` is limited to driving the UI primarily through the keyboard. For example, `desktop.OpenWindow()` works by closing all windows, then using type-down selection on each path segment followed by the OA+SA+Down shortcut to open the selection while closing the current window. This is generally sufficient but some things require creativity:
  * As noted, methods like `desktop.SelectPath()` and `desktop.OpenWindow()` by default close all windows first. If you need multiple windows open for an operation you need get creative.
    * Add a shortcut (e.g. via `desktop.AddShortcut()`) for a dependent window, use `desktop.OpenWindow()` to open the initial window, then `a2d.OAShortcut("1")` (etc) to open the second window.
